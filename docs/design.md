...
- MAX `on_message_delete` → MessageLink → в Topic уходит Note «🗑️ удалено!» (эмодзи U+1F5D1 с селектором U+FE0F, затем пробел U+0020) ответом на
  ...
  прочитать текст сообщения, а хранить его не хотим). Telegram об удалениях Owner
  не сообщает, обратного пути нет.
...
| `/history N` | Topic | Подгрузить последние N сообщений, N обязателен |
