...
| `PATCH /accounts/{id}/chats/{link_id}/topic` | Переименовать существующий Topic: JSON `{"name": "Новое название"}`, 1–128 символов после обрезки краевых пробелов; при успехе `renamed_by_owner = true` |
...
3. Находится или создаётся ChatLink. Topic создаётся при первом сообщении: имя из `Chat.title`
   для Group и Channel, из имени и фамилии собеседника для Dialog без `title`
   (если фамилии нет — только имя, если нет имени — телефон или `MAX <id>`); цвет иконки
   по виду MaxChat: Dialog светло-зелёный `0x8EEE98`, Group светло-синий `0x6FB9F0`,
   Channel оранжевый `0xFB6F5F`. Перед созданием Topic Dialog активный runner уточняет
   имя через `get_user`, даже если ChatLink уже хранит старое имя. Без runner используется
   сохранённое имя. Существующие Topic из-за изменения сборки имени не переименовываются.
4. Собирается `RelayMessage` (раздел 7): текст с сущностями форматирования, вложения, ссылка
   на ответ через MessageLink, жирная подпись отправителя в Group и Dialog.
...
- Operator в «Чаты MAX» нажимает «Переименовать», вводит название и сохраняет его.
  Доступно только для ChatLink с Topic и подключённым Inbox. API вызывает `editForumTopic`
  через адаптер Telegram, затем сохраняет `renamed_by_owner = true`; `max_title` остаётся
  названием MAX. При отказе Telegram флаг не меняется, UI показывает безопасный текст
  без тела ответа и credentials. Отсутствующий Topic и некорректное название дают 400,
  чужой или отсутствующий ChatLink — 404, ошибка Telegram — 502.
  Создание, ручное и автоматическое переименование используют общую блокировку ChatLink:
  одновременное обновление MAX не перезапишет ручное название. Служебный апдейт от правки
  через UI может повторно выставить тот же флаг; это безопасно и не мешает распознаванию
  автоматических правок runner.
...
| Пересланное (`ForwardLink`) | Префикс «↪️ Переслано от X» перед содержимым; X — полное имя доступного отправителя исходного сообщения, а если отправитель скрыт — название исходного чата или «неизвестного отправителя» |
...
и Dialog каждое сообщение начинается с жирного имени отправителя. `first_name` и `last_name`
из `User.names` собираются через пробел; если фамилии нет, остаётся только имя. `name`
используется как запасной вариант при отсутствии `first_name`, уже полное имя не дублируется.
Правило действует и для подписей пересланных сообщений и при правках. Сообщения Owner
...
UI Operator использует Streamlit 1.63.0 и streamlit-shadcn-ui 1.4.0 (Components V2,
Shadow DOM без iframe). Кнопки списка чатов — shadcn V2. Account показаны компактной адаптивной сеткой
карточек с названием, цветным состоянием и телефоном. Вся карточка — область native-кнопки
с поддержкой клавиатуры; отдельной кнопки открытия нет.
В пользовательских подписях Account называется «учетка». Форма входа Operator
использует встроенную форму Streamlit: пароль отправляется кнопкой или Enter.
В «Чаты MAX» действия строки сгруппированы
справа: «Создать Topic» либо «Переименовать» в стиле outline (как «Пауза») и Mute/Unmute в стиле secondary.
Список отрисовывается общими Elements-деревьями по 50 строк, без пагинации: это ограничивает
число V2-компонентов и сохраняет доступность последних строк и вкладок при автообновлении.
На узком экране (390 px) группа переносится целиком под название. Форма нового названия
открывается под строкой, с кнопками «Сохранить» и «Отмена»; при недоступном bridge
сохранение заблокировано. Это позволяет менять название Topic и в личном Inbox,
где клиент Telegram не предлагает переименование.
...
Telegram-сторона уже проверена 13.09.2026 на тестовом боте: создание Topic в личном чате,
