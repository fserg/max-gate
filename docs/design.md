...
- Форматирование только MAX → Telegram; текст из Telegram уходит в MAX экранированным.
...
Не входит (v2): реакции, форматирование Telegram → MAX, начало разговора из Telegram
...
3. Текст экранируется от markdown-разметки PyMax (символы `* _ ~ \` [ ] # >`), сущности
   Telegram в v1 не переносятся. Вложения скачиваются через `getFile` (до 20 МБ, иначе Note).
...
- MAX `on_message_delete` → MessageLink → текст или подпись заменяются на «🗑 удалено».
  Telegram об удалениях Owner не сообщает, обратного пути нет.
...
|---|---|
| Текст | Экранированный текст, без форматирования |
...
- Чаты без ChatLink при первом запуске Account получают `last_relayed_time = now` и историю
  не подтягивают.
- `/history N` в Topic: `fetch_history(backward=N)`, N по умолчанию 20, не больше 100;
  сообщения уходят с временем и без записи в MessageLink.
...
| `/history N` | Topic | Подгрузить последние N сообщений |
...
maxgate/
  ...
  domain/            чистая логика без I/O: RelayMessage, рендер подписей и Note,
                     экранирование, разбиение текста, политика повторов
...
| Реакции из MAX сервер не присылает | Реакции отложены на v2 |
