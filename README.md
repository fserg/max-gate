...
в Topic. Доступны `/status`, `/mute`, `/unmute`, `/history N`, `/reload N`, `/delete` ответом на сообщение и `/info`.
...
`/reload N` заново отправляет последние N сообщений MAX, даже если они уже отражались.
N обязателен, положительный, максимум 100. Используйте команду, если удалили копии в Telegram
и хотите восстановить их. После успешной отправки каждого сообщения старые MessageLink заменяются
новыми: правки и удаления из MAX, ответы из Telegram идут через свежую копию. Оставшиеся старые
копии больше не связаны с MAX; при ошибке переотправки прежние связи сохраняются. `/history N`
по-прежнему пропускает уже отражённые сообщения. Обе команды сохраняют порядок `(time, id)` и
префикс времени; ошибка одного сообщения не останавливает остальные, живые события ждут историю.

`/delete` отправляется **ответом на сообщение в Topic**. Gate находит MessageLink и запрашивает
удаление сообщения в MAX **для всех участников**. При успехе удаляет связанные части в Telegram
и саму команду; собственный Echo удаления не создаёт «🗑️ удалено!». При отказе MAX (например,
нельзя удалить чужое сообщение) копии остаются, приходит Note «❌ причина». Без ответа или без
MessageLink бот даёт подсказку. Если MAX уже подтвердил удаление, а очистка Telegram не удалась,
повторяется только очистка; после исчерпания повторов приходит Note. Возможность удаления старых
копий зависит от ограничений и прав Telegram-бота. Обычное удаление копии вручную в Telegram
не удаляет оригинал MAX — Bot API не присылает такое событие.
...
Для ChatLink с Topic рядом с Mute есть кнопка «Переименовать». Введите новое название
(1–128 символов) и нажмите «Сохранить». Это работает и для Topic в личном чате бота.
После успешного переименования изменения названия в MAX больше не перезаписывают Topic.
Ошибки Telegram показываются безопасным текстом. Внутренний endpoint:
`PATCH /accounts/{id}/chats/{link_id}/topic`, JSON `{"name": "Новое название"}`.

## Форматирование и удаление

MAX → Telegram в Group и Dialog добавляет жирную подпись отправителя: имя и фамилию,
если заполнены оба поля, иначе только имя. Для новых Topic Dialog используется то же
полное имя собеседника; существующие Topic автоматически не переименовываются.
Сообщения Owner с телефона по-прежнему подписываются «Вы».
...
Внешнее удаление сообщения в MAX оставляет исходное сообщение Telegram неизменным.
...
Исключение — собственное удаление командой `/delete`: копии удаляются, Echo не создаёт Note.

Реакции обрабатываются отдельной очередью на ChatLink, последовательно между собой,
но независимо от очереди сообщений. Задания реакций не повторяются с
задержками 1/5/30. При отказе MAX принять emoji Gate делает одну дополнительную попытку
с переключённым U+FE0F (добавляет или убирает селектор). Если вариант принят, ошибка
не показывается. Если оба варианта отвергнуты, Gate один раз снимает прежнюю свою реакцию
(включая установленную до перезапуска) и отвечает: «⛔ MAX не поддерживает реакцию …,
реакция в MAX снята». Если снять её не удалось, ответ сообщает об этом без утверждения,
что реакция удалена.

Для всех заданий окончательные ApiError MAX, включая ошибки отправки и правки, не
повторяются. Классификация общая, по коду ошибки; неизвестный код отказа считается
окончательным. Явные коды timeout/потери соединения перечислены в
`maxgate/relay/errors.py`; транспортные ошибки и таймауты сообщений сохраняют повторы.
