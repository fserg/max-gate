...
Двусторонний перенос сообщений из MAX в Telegram и обратно через телеграм-бота с топиками.
...
Временные медиа удаляются после обработки задания. Очереди, незавершённые альбомы и подписи частей доставленных альбомов
...
`maxgate/max/uploads.py` заменяет загрузку File, Photo, Video и Voice этого клиента: TLS
...
При истечении срока дренирования задания в памяти теряются. Gate пишет WARNING с числом
недоставленных заданий каждого направления; для Telegram → MAX пытается ответить Owner
сообщением «❌ …» (до 5 секунд на уведомления). После аварийного завершения уведомления
не гарантированы; подтверждённые Telegram-апдейты автоматически не восстанавливаются.

Один Telegram-бот может принадлежать только одному Account (`tg_bot_id`, UNIQUE).
Миграция `0002` заполняет ID существующих ботов из расшифрованных токенов без сети;
при дублирующихся ботах она останавливается до изменения схемы. Требуется исходный
`MAXGATE_SECRET_KEY`. `seed-account` также заполняет ID бота.
Повторный `/start` из другого чата не меняет Inbox: перепривязку выполняет Operator
сменой режима Inbox или Owner в настройках Account; прежние Topic и MessageLink сбрасываются.
Правка части альбома сохраняет подписи остальных частей в течение жизни runner;
после перезапуска остаётся прежнее поведение правки по одному сообщению.

## Topic, история и реакции

В «Чаты MAX» кнопка «Создать Topic» доступна для ChatLink без Topic. Требуется
подключённый Inbox; повторный запрос возвращает существующий Topic. Endpoint:
`POST /accounts/{id}/chats/{link_id}/topic`. Имя и цвет совпадают с автоматическим созданием.

Команда `/history N` требует положительное число N (максимум 100). История получает
префикс времени и MessageLink: работают ответы, правки и удаления. Уже отражённые
сообщения повторно не отправляются. Ручная история не продвигает курсор Catch-up.

Реакции Owner в Telegram передаются в MAX через MessageLink; снятие удаляет свою
реакцию MAX. Поддерживается одна обычная emoji-реакция; MAX проверяет допустимость.
При отсутствии связи, неподдерживаемом типе или ошибке появляется одно сообщение «⛔ …».
PyMax не задаёт фиксированный перечень emoji: живая проверка подтвердила `👍` и отказ
для `🧪` с `error.message.like.unknown.like`. Зонд от 13.09.2026 не получил события
реакции собеседника (opcode 155), поэтому направление MAX → Telegram не включено.

## Форматирование и удаление

Telegram → MAX сохраняет текст без Markdown-разбора и передаёт сущности отдельно:
`bold→STRONG`, `italic→EMPHASIZED`, `underline→UNDERLINE`,
`strikethrough→STRIKETHROUGH`, `code→MONOSPACED`, `pre→CODE`,
`text_link/url→LINK`, `blockquote→QUOTE`. Смещения и длины — в UTF-16;
неподдерживаемые сущности и некорректные диапазоны пропускаются. Форматирование
сохраняется при отправке, правках, объединении подписей альбома и разбиении текста.

Удаление сообщения в MAX оставляет исходное сообщение Telegram неизменным.
В Topic приходит Note **«🗑️ удалено!»** ответом на первую часть исходного сообщения.
Если оригинал уже отсутствует в Telegram, Note отправляется в тот же Topic без reply.
